...
for (i in 2:nrow(mps)) {
  ...
  if (mps$data.currentParty[i] == "Bloc Québécois") {
    mps$data.currentParty[i] <- "BQ"
  }
  
  if (mps$data.currentParty[i] == "Coalition avenir Québec") {
    mps$data.currentParty[i] <- "CAQ"
  }
  
  if (mps$data.currentParty[i] == "Conservative") {
    mps$data.currentParty[i] <- "CPC"
  }
  
  if (mps$data.currentParty[i] == "Green Party") {
    mps$data.currentParty[i] <- "GPC"
  }
  
  if (mps$data.currentParty[i] == "Indépendant") {
    mps$data.currentParty[i] <- "Independent"
  }
  
  if (mps$data.currentParty[i] == "Indépendante") {
    mps$data.currentParty[i] <- "Independent"
  }
  
  if (mps$data.currentParty[i] == "Liberal") {
    mps$data.currentParty[i] <- "LPC"
  }
  
  if (mps$data.currentParty[i] == "Parti conservateur du Québec") {
    mps$data.currentParty[i] <- "PCQ"
  }
  
  if (mps$data.currentParty[i] == "Parti libéral du Québec") {
    mps$data.currentParty[i] <- "PLQ"
  }
  
  if (mps$data.currentParty[i] == "Parti québécois") {
    mps$data.currentParty[i] <- "PQ"
  }
  
  if (mps$data.currentParty[i] == "Québec solidaire") {
    mps$data.currentParty[i] <- "QS"
  }
  ...
  data_to_commit <- list(firstName = mps$data.firstName[i], 
    ...
    metadata_to_commit <- list(source = mps$metadata.source[i], 
                               ...
                               institution = mps$metadata.institution[i])
  #} else {
  #  metadata_to_commit <- list(source = mps$metadata.source[i], 
  #                             country = mps$metadata.country[i],
  #                             province_or_state = mps$metadata.province_or_state[i],
  ...
  clessnhub::edit_item('persons', key = mps$key[i], type = mps$type[i], schema = mps$schema[i], metadata_to_commit, data_to_commit)
  cat("\n")
  cat("===========================================================================\n")
