...
cd /Users/patrick/Dev/CLESSN/clessn-blend/agoraplus-canada/s_agoraplus_canada

if [[ -f "status.json" ]]; then
   result=`cat status.json`
   echo "$(date) starting s_agoraplus-canada . current job status: " $result
else
   result='"not running"'
   echo "$(date) starting s_agoraplus-canada . current job status: " $result
fi
...
curl -s -X POST -H "Content-Type: application/json" -d '{"commands": ["Rscript", "code/code.R"], "env_vars": {"HUB_TOKEN":"'"$HUB_TOKEN"'", "HUB3_USERNAME":"'"$HUB3_USERNAME"'", "HUB3_PASSWORD":"'"$HUB3_PASSWORD"'", "LOG_PATH":"."}}' http://localhost:8080/run 
...
if [[ $ret = '1' ]]; then
   echo "$(date) job terminated with an error.  Error log generated in /Users/patrick/Logs/s_agoraplus-canada-$(date +%Y-%m-%d.%H:%M:%S).log"
   cat return_code.json stderr.json stdout.json > "/Users/patrick/Logs/s_agoraplus-canada-$(date +%Y-%m-%d.%H:%M:%S).log"
fi

if [[ $ret = '2' ]]; then
   echo "$(date) job terminated with warning(s).  Error log generated in /Users/patrick/Logs/s_agoraplus-canada-$(date +%Y-%m-%d.%H:%M:%S).log"
   cat return_code.json stderr.json stdout.json > "/Users/patrick/Logs/s_agoraplus-canada-$(date +%Y-%m-%d.%H:%M:%S).log"
fi
...
docker stop s_agoraplus-canada-pipeline-1 
