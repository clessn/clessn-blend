...
cd /Users/patrick/Dev/CLESSN/clessn-blend/Pipelines/Loaders/l_pressreleases_qc

if [[ -f "status.json" ]]; then
   result=`cat status.json`
   echo "$(date) starting l_pressreleases_qc. current job status: " $result
else
   result='"not running"'
   echo "$(date) starting l_pressreleases_qc. current job status: " $result
fi
...
if [[ $ret = '1' ]]; then
   echo "$(date) job terminated with an error.  Error log generated in /Users/patrick/Logs/l_pressreleases_qc-$(date +%Y-%m-%d.%H:%M:%S).log"
   cat return_code.json stderr.json stdout.json > "/Users/patrick/Logs/l_pressreleases_qc-$(date +%Y-%m-%d.%H:%M:%S).log"
fi

if [[ $ret = '2' ]]; then
   echo "$(date) job terminated with warning(s).  Error log generated in /Users/patrick/Logs/l_pressreleases_qc-$(date +%Y-%m-%d.%H:%M:%S).log"
   cat return_code.json stderr.json stdout.json > "/Users/patrick/Logs/l_pressreleases_qc-$(date +%Y-%m-%d.%H:%M:%S).log"
fi
