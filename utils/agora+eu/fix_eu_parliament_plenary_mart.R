...
df_duplicates <- read.csv("DataAgora_doublons.csv")

for (i in 576:nrow(df_duplicates)) {
  my_filter <- list(data__intervention_id = df_duplicates$intervention_id[i])
  mart_items <- clessnverse::get_mart_table(table_name, credentials, my_filter, nbrows = 0)

  if (nrow(mart_items) == 2) {
    if ( nchar(mart_items$hub.key[1]) <  nchar(mart_items$hub.key[2]) ) {
      key_to_trash <- mart_items$hub.key[1]
    } else {
      key_to_trash <- mart_items$hub.key[2]
    }

    index <- which(mart_items$hub.key == key_to_trash)

    cat("deleting", key_to_trash, "with id", mart_items$hub.id[index], "\n")

    hublot::remove_table_item(paste("clhub_tables_mart_", table_name, sep=''), mart_items$hub.id[index], credentials)

  } else {
    cat("!!! ERROR !!! dataframe has more than 2 rows.  Filter is: ", paste(my_filter, collapse = " "), "\n")
  }
}
