...
if (scraping_method == "Latest") {
  ...
  #i_get_attempt <- 1
  #while (is.null(source_page)  && i_get_attempt <= 20) { source_page <- httr::GET(paste(base_url,content_url,sep=''), httr::config(ssl_verifypeer=0), httr::timeout(30)) }

  #source_page_html <- httr::content(source_page)
  source_page_html <- xml2::read_xml(paste(base_url,content_url,sep=''))
...
for (i_url in 1:length(urls_list_fr)) {
  ...
  i_get_attempt <- 1
  while(is.null(r_fr) && i_get_attempt <= 20) { r_fr <- httr::GET(current_url_fr, httr::config(ssl_verifypeer=0), httr::timeout(30)) } #safe_GET(current_url_fr) 

  if (r_fr$status_code == 200) {
    current_url_en <- urls_list_en[[i_url]]

    i_get_attempt <- 1
    while(is.null(r_en) && i_get_attempt <= 20) { r_en <- httr::GET(current_url_en, httr::config(ssl_verifypeer=0), httr::timeout(30)) }
    ...
    if (r_en$status_code == 200) {
      ...
      clessnverse::logit(scriptname, paste("Could not GET", current_url_en, "error code", r_fr$status_code, sep = " "), logger)
      next  
    }
    ...
    clessnverse::logit(scriptname, paste("Could not GET", current_url_fr, "error code", r_fr$status_code, sep = " "), logger)
    next
  }
