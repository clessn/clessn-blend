...
extract_press_release <- function(party_acronym, xml_root) {
    #".//header[@class = 'mb-3']"
    ...
    df_press_release <- data.frame(title = title,
                                   date = date,
                                   body = body)

    return(df_press_release)
}
...
load_press_release_to_dw <- function(key, party_acronym, lake_file_url) {
    ...
    if (r$result$status_code == 200) {
        ...
        index_html <- gsub("&rsquo;", "'", index_html)
        ...
        press_releases <- extract_press_release(party_acronym, xml_root)

        clessnverse::commit_warehouse_row(warehouse_table, key, press_releases, mode = opt$hub_mode, credentials)
...
main <- function() {
    ...
    clessnverse::logit(scriptname,  paste("loading", length(items_list) , "items to the data warehouse",  sep=" "), logger)
    ...
    for (i in 1:length(items_list$key)) {
        ...
        clessnverse::logit(scriptname,  paste("loading item #", i, "key = ", items_list$key[i], sep=" "), logger)

        load_press_release_to_dw(key, party_acronym, lake_file_url)
    }
...
tryCatch( 
  {
    ...
    lake_path <- "political_party_press_releases"
