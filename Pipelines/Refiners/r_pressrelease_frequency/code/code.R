...
main <- function() {
    ...
    warehouse_press_release_table <- "political_parties_press_releases"
    datamart_press_release_frequency <- "political_parties_press_releases_frequency"
    ...
    #
    warehouse_df <- clessnverse::get_warehouse_table(warehouse_press_release_table, credentials)
    ...
    clessnverse::commit_mart_table(datamart_press_release_frequency, datamart_df,
                                   key_column = "key", mode = "refresh", credentials)
...
tryCatch(
  withCallingHandlers(
  {
    ...
    if (!exists("scriptname")) scriptname <<- "r_pressrelease_frequency"
