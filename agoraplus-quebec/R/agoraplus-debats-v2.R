...
opt <- list(cache_mode = "rebuild",simple_mode = "rebuild",deep_mode = "rebuild", 
            dataframe_mode = "refresh", hub_mode = "skip")
...
oob_rubric_patterns <- c("^Affaires courantes", "^Affaires du jour", "^Ajournement")
oob_title_patterns <- c( "^Affaires prioritaires", "^Affaires prioritaires (suite)", "^Déclarations de députés", 
...
list_urls <- c("/fr/travaux-parlementaires/assemblee-nationale/42-1/journal-debats/20200218/264427.html")
...
for (i in 1:length(list_urls)) {
  ...
  if (grepl("/travaux-parlementaires/assemblee-nationale/", event_url)) {     
    ...
    if ( version_finale && 
      ...
      for (x in 0:start.index) doc_text[x] <- NA
      ...
      if ( event_end_time[length(event_end_time)] == "heures" ) {
      ...
      for (j in 1:length(doc_text)) {
        ...
        #matching_row <- which(dfDeep$eventID == event_id & dfDeep$interventionSeqNum == intervention_seqnum)
        ...
        intervention_start <- substr(doc_text[j],1,55)
        
        if ( grepl("(^\\•\\s\\()|(^\\()", intervention_start) ) {
          sob_procedural_text <- doc_text[j]
          sob_procedural_text <- gsub("(^\\•\\s\\()|(^\\()|\\)\\s\\•$|^\\(|\\)$", "", sob_procedural_text)
          if (j < length(doc_text[j])) next
        } 
        
        if ( grepl("^Titre(.*?): ", intervention_start) ) {
          title <- gsub("^Titre : ", "", doc_text[j])
          if ( TRUE %in% stringr::str_detect(title, oob_rubric_patterns) ) {
            oob_rubric <- title
          } else {
            if ( TRUE %in% stringr::str_detect(title, oob_title_patterns) ) {
              oob_title <- title
            } else {
        ...
        if ( (TRUE %in% stringr::str_detect(intervention_start, patterns_intervenants)) &&
             !grepl(",", stringr::str_match(intervention_start, "^(.*):")) &&
             stringr::str_detect(intervention_start, "^(.*):")
           ) {
          ...
          if ( grepl("^(le|la)(\\s+)?(vice(\\s|\\-))?président(.*):", tolower(intervention_start)) ||
               grepl("^(m.|mme)(.*)\\((président|présidente)(.*)\\)(.*):", tolower(intervention_start))
             ) { ### MODERATEUR ###
            ...
            if ( grepl("Mme", intervention_start) || grepl("M\\.", intervention_start) ) 
              speaker_last_name <- clessnverse::removeSpeakerTitle(stringr::str_match(intervention_start, "\\((.*)\\)\\s+:")[2])
            
            if ( grepl("^président(e?)", tolower(speaker_last_name)) ) {
              speaker_type <- speaker_last_name
              speaker_last_name <-  stringr::str_match(intervention_start, "^(M\\.|Mme)\\s+(.*)\\s+\\((.*)\\)\\s+:")[3]
              if ( grepl("^M\\.", intervention_start) ) {
                gender_femme <- 0
              } else {
                gender_femme <- 1
              }
            ...
            if ( grepl("^le(\\s+)?président(.*):", tolower(intervention_start)) ||
                 grepl("^la(\\s+)?présidente(.*):", tolower(intervention_start)) ) {
              speaker_type <- "président(e)"
              speaker_last_name <- "Paradis"
              speaker_first_name <- "François"
              speaker_gender <- "M"
              #speaker <- subset(deputes, grepl(paste(tolower(speaker_first_name),tolower(speaker_last_name),sep=" "),tolower(nom)))
              speaker <- dplyr::filter(deputes, tolower(firstName) == tolower(speaker_first_name) & (tolower(lastName1) == tolower(speaker_last_name) | tolower(lastName2) == tolower(speaker_last_name)))
              
            } else {
              if ( grepl("^le(\\s+)?vice-président(.*):", tolower(intervention_start)) ||
                   grepl("^la(\\s+)?vice-présidente(.*):", tolower(intervention_start)) ) {
                speaker_type <- "vice-président(e)"
                
                if ( grepl("mme(.*):", tolower(intervention_start)) ) gender_femme <- 1 else gender_femme <- 0
                    
                speaker_last_name <- clessnverse::removeSpeakerTitle(stringr::str_match(intervention_start, "\\((.*)\\)\\s+:")[2])
                
                #speaker <- subset(deputes, grepl(tolower(speaker_last_name), tolower(nom)) & grepl(gender_femme, femme))
                ln1 <- stringr::word(speaker_last_name, 1)
                ln2 <- stringr::word(speaker_last_name, 2)
                if (is.na(ln2)) {
                  speaker <- dplyr::filter(deputes, (tolower(lastName1) == tolower(ln1) | tolower(lastName2) == tolower(ln1)) & isFemale == gender_femme)
            ...
            if (  1 %in% match(patterns_periode_de_questions, tolower(c(intervention_start)),FALSE) ) periode_de_questions <- TRUE
            
            if ( nrow(speaker) > 0 ) { 
              speaker_last_name <- paste(na.omit(speaker[1,]$lastName1), na.omit(speaker[1,]$lastName2), sep = " ")
              speaker_last_name <- trimws(speaker_last_name, which = c("both"))
              if (length(speaker_last_name) == 0) speaker_last_name <- NA
              speaker_first_name <- speaker[1,]$firstName
              speaker_gender <- if ( speaker[1,]$isFemale == 1) "F" else "M"
              if ( tolower(speaker[1,]$party) == "fonctionnaire" ) {
                ...
                speaker_type <- "élu(e)"
                speaker_party <- speaker[1,]$party
                speaker_district <- speaker[1,]$currentDistrict
              }
              
              speaker_is_minister <- speaker$isMinister[1]
            } # ( nrow(speaker) > 0 ) 
            
            intervention_type <- "modération"
            intervention_text <- substr(doc_text[j], unlist(gregexpr(":", intervention_start))+1, nchar(doc_text[j]))
            ...
            if ( grepl("Mme", stringr::str_match(intervention_start, "(.+)\\s+:")[1]) || 
              ...
              if ( grepl("\\(", stringr::str_match(intervention_start, "(.+)\\s+:")[1]) ) {
                ...
                district_or_lastname <- clessnverse::removeSpeakerTitle(stringr::str_match(intervention_start, "\\((.*)\\)")[2])
                ...
                if ( grepl("Mme", stringr::str_match(intervention_start, "(.+)\\s+:"))[1] ) {
                  speaker_last_name <- stringr::str_match(intervention_start, "Mme\\s+(.*?)\\s+\\(")[2]
                  gender_femme <- 1
                }
                else {
                  speaker_last_name <- stringr::str_match(intervention_start, "M\\.\\s+(.*?)\\s+\\(")[2]
                  gender_femme <- 0
                }
              ...
              if ( is.na(speaker_district) ) {
               if (is.na(ln2)) {
                ...
                if (is.na(ln2)) {
                  speaker <- dplyr::filter(deputes, (tolower(lastName1) == tolower(ln1) | tolower(lastName2) == tolower(ln1)) & tolower(currentDistrict) == tolower(speaker_district))
                } else {
                  speaker <- dplyr::filter(deputes, (tolower(lastName1) == tolower(ln1) & tolower(lastName2) == tolower(ln2)) & tolower(currentDistrict) == tolower(speaker_district))
                }
              }
        ...
        if ( ((TRUE %in% stringr::str_detect(substr(doc_text[j+1],1,55), patterns_titres)) &&
              !grepl(",", stringr::str_match(substr(doc_text[j+1],1,55), "^(.*):")[1])) &&
              stringr::str_detect(substr(doc_text[j+1],1,55), "^(.*):") &&
              !is.na(doc_text[j]) || 
